...
# length = {'trainval': 3130, 'mini': 323}
# length = {'trainval': 7742, 'mini': 323} # train: 5772, val: 1970, test:1253
...
log_level = "INFO"
work_dir = "work_dirs/6cams_both_infra_v6_stage2_ego_filter"
...
# ================== model ========================
with_infra_cam = True
infra_cam_select = 0  # 0: use both infrastructure cameras, 1: cam1 only, 2: cam2 only
num_cams = 6 if with_infra_cam else 4  # 4 vehicle + 2 infrastructure
    ...
    head=dict(
        ...
        det_head=dict(
            ...
            instance_bank=dict(
                ...
                num_temp_instances=700 if temporal else -1,  # Increased from 600: more memory slots reduce ID drops
                confidence_decay=0.92,  # Increased from 0.85: slower decay keeps instances in top-K longer
                feat_grad=False,
            ),
            anchor_encoder=dict(
            ...
            decoder=dict(
                ...
                # score_threshold=0.2,  # TODO: re-enable after confirming model detection works
            ),
...
data_basic_config = dict(
    ...
    infra_cam_select=infra_cam_select,  # Use both infrastructure cameras
)
eval_config = dict(
    **data_basic_config,
...
data = dict(
    ...
    val=dict(
        **data_basic_config,
        ...
        samples_per_gpu=1,  # CRITICAL: Must be 1 for proper tracking!
    ),
    test=dict(
        **data_basic_config,
        ann_file=anno_root + "nuscenes_infos_test.pkl",
...
# ================== pretrained model ========================
load_from = 'work_dirs/6cams_both_infra_v6/latest.pth'
# resume_from = 'work_dirs/6cams_both_infra_v6_stage2/latest.pth'