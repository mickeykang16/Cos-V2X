...
class Sparse4DHead(BaseModule):
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(instance_bank, nn.Module):
    ...
    def _forward_with_bank_io(
        self,
        feature_maps: Union[torch.Tensor, List],
        metas: dict,
        bank_io: tuple,
    ):
        """Core forward logic given a pre-fetched bank state.

        Args:
            feature_maps: camera feature maps (list or tensor)
            metas: metadata dict
            bank_io: tuple of (instance_feature, anchor,
                      temp_instance_feature, temp_anchor, time_interval)
                      as returned by instance_bank.get()

        Returns:
            output (dict): head outputs (classification, prediction, etc.)
            instance_feature (Tensor): final (bs, N, C) for bank caching
            anchor (Tensor): final (bs, N, anchor_dim) for bank caching
            cls (Tensor): final classification scores for bank caching
        """
        ...
        return output, instance_feature, anchor, cls
    ...
    def forward(
        ...
        # Reset stale dn_metas before passing to bank.get()
        if (
        ...
        bank_io = self.instance_bank.get(
            batch_size, metas, dn_metas=self.sampler.dn_metas
        )
        output, instance_feature, anchor, cls = self._forward_with_bank_io(
            feature_maps, metas, bank_io
        )
    ...
    def forward_shared(
        ...
        feature_maps: Union[torch.Tensor, List],
        metas: dict,
        shared_bank_state: tuple,
    ):
        """Forward using a pre-fetched shared bank state.

        Like ``forward()`` but uses the provided bank state instead of
        calling ``instance_bank.get()``.  Does NOT call
        ``instance_bank.cache()`` or ``instance_bank.get_instance_id()``;
        the parent (SparseDriveHeadV2X) is responsible for those calls.

        Args:
            feature_maps: camera feature maps
            metas: metadata dict
            shared_bank_state: tuple returned by ``shared_instance_bank.get()``

        Returns:
            Tuple[dict, Tensor, Tensor, Tensor]:
                output dict, final instance_feature, final anchor, final cls
        """
        if isinstance(feature_maps, torch.Tensor):
            feature_maps = [feature_maps]
        batch_size = feature_maps[0].shape[0]

        # Ensure dn_metas matches current batch size (same guard as forward())
        if (
            self.sampler.dn_metas is not None
            and self.sampler.dn_metas["dn_anchor"].shape[0] != batch_size
        ):
        ...
        return self._forward_with_bank_io(
            feature_maps, metas, shared_bank_state
        )
