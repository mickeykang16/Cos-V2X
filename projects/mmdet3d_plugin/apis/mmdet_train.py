...
def custom_train_detector(
    ...
    if distributed:
        ...
        # _set_static_graph() allows gradient checkpointing (with_cp=True) to
        # coexist with DDP when the same backbone features flow to multiple heads.
        # Required when a checkpoint-wrapped backbone is used more than once in
        # the backward graph (e.g. V2X infra features → det head + map head).
