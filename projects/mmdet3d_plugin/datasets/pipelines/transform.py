...
class NuScenesSparse4DAdaptor(object):
    ...
    def __call__(self, input_dict):
        ...
        for key in [
            ...
            'gt_ego_fut_cmd',
            'ego_status',
            'gt_ego_fut_trajs_2hz', # Add reference to 2hz trajectory
            'gt_ego_fut_masks_2hz', # Add reference to 2hz mask
        ]:
            if key not in input_dict:
                continue
            input_dict[key] = DC(to_tensor(input_dict[key]), stack=True, cpu_only=False, pad_dims=None)
        
        if 'gt_ego_fut_trajs' in input_dict:
             # Just duplicate for now as they are likely same hz
             input_dict['gt_ego_fut_trajs_2hz'] = input_dict['gt_ego_fut_trajs']
             input_dict['gt_ego_fut_masks_2hz'] = input_dict['gt_ego_fut_masks']
        
        if 'ego_status' in input_dict and 'ego_status_mask' not in input_dict:
             input_dict['ego_status_mask'] = DC(to_tensor(np.ones_like(input_dict['ego_status'].data)), stack=True, cpu_only=False, pad_dims=None)
