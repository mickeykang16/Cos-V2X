...
def planning_eval(results, eval_config, logger):
    ...
    planning_metrics = PlanningMetric(n_future=12)
    for i, data in enumerate(tqdm(dataloader)):
        ...
        planning_metrics.update(pred_sdc_traj[:, :12, :2], sdc_planning[0,:, :12, :2], sdc_planning_mask[0,:, :12, :2], fut_boxes)
    ...
    for key in planning_results.keys():
        ...
        value = new_values
        avg = [value[1], value[3], value[5], value[7], value[9], value[11]]
        avg = sum(avg) / len(avg)
        value.append(avg)
        metric_dict[key] = avg
        row_value = []
