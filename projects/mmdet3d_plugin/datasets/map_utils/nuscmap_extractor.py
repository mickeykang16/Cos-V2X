...
import sys, os
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../../../UniV2X_REAL/projects/mmdet3d_plugin/datasets/eval_utils')))
from map_api import NuScenesMap, NuScenesMapExplorer
...
class NuscMapExtractor(object):
    ...
    def get_map_geom(self, 
        ...
        # HH
        lane_lines = []
        for lane_polygon in lanes:
            if lane_polygon.geom_type == 'MultiPolygon':
                # MultiPolygon일 경우, 포함된 각 Polygon의 exterior를 추가
                for p in lane_polygon.geoms:
                    lane_lines.append(p.exterior)
            else:
                # Polygon일 경우, 기존과 동일하게 exterior를 추가
                lane_lines.append(lane_polygon.exterior)
        
        road_segment_lines = []
        for road_segment_polygon in road_segments:
            if road_segment_polygon.geom_type == 'MultiPolygon':
                for p in road_segment_polygon.geoms:
                    road_segment_lines.append(p.exterior)
            else:
                road_segment_lines.append(road_segment_polygon.exterior)
        ...
        return dict(
            ...
            lane = lane_lines,
            intersection = road_segment_lines,
        )
